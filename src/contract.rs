...
use crate::state::{lottery, lottery_read, read_viewing_key, write_viewing_key, Config, Constants, Lottery, ReadonlyConfig, last_lottery_results, LastLotteryResults, last_lottery_results_read};
...
pub fn init<S: Storage, A: Api, Q: Querier>(
    ...
    let last_lottery = LastLotteryResults {
        ...
        past_total_rewards: vec![],
    };
    let _ = last_lottery_results(&mut deps.storage).save(&last_lottery);

    // Setting Total supply
    TypedStoreMut::<SupplyPool, S>::attach(&mut deps.storage).store(
        SUPPLY_POOL_KEY,
        &SupplyPool {
...
pub fn handle<S: Storage, A: Api, Q: Querier>(
    ...
    match contract_status {
        ContractStatusLevel::StopAll | ContractStatusLevel::StopAllButWithdraw => {
            let response = match msg {
                ...
                if contract_status == ContractStatusLevel::StopAllButWithdraw =>
                    {
    ...
    let response = match msg {
        ...
        //Triggerer
        HandleMsg::ClaimRewards {} => claim_rewards(deps, env),                                                          //tested

        // Stakepool's Functions
        HandleMsg::Deposit { .. } => try_deposit(deps, env),                                                              //tested
        HandleMsg::Redelegate { amount, .. } => try_redelegate(deps, env, amount),              //tested
        HandleMsg::Withdraw { amount, .. } => try_withdraw(deps, env, amount),                    //tested
        HandleMsg::TriggerWithdraw { amount, .. } => trigger_withdraw(deps, env, amount),   //tested
        HandleMsg::TriggeringCostWithdraw {} => triggering_cost_withdraw(deps, env),                                //tested

        // Base
        HandleMsg::CreateViewingKey { entropy, .. } => try_create_key(deps, env, entropy),                       //tested
        HandleMsg::SetViewingKey { key, .. } => try_set_key(deps, env, key),                                     //tested

        // Admin
        HandleMsg::ChangeAdmin { address, .. } => change_admin(deps, env, address),                      //tested
        HandleMsg::ChangeTriggerer { address, .. } => change_triggerer(deps, env, address),              //tested
        HandleMsg::ChangeTriggererShare { percentage,.. } => change_triggerer_share(deps, env, percentage),          //tested
        HandleMsg::ChangeLotteryDuration {duration}=> change_lottery_duration(deps, env,duration),                              //NOT TESTED
        HandleMsg::ChangeUnbondingTime {unbonding_time}=> change_unbonding_time(deps, env,unbonding_time),                              //NOT TESTED


        HandleMsg::ChangeValidator {address}=>change_validator(deps,env,address),
        HandleMsg::SetNormalStatus {} => set_normal_status(deps, env),                                              //tested
        HandleMsg::SetStopAllStatus {} => set_stop_all_status(deps, env),                                           //tested
        HandleMsg::SetStopAllButWithdrawStatus {} => set_stop_all_but_withdraw_status(deps, env),                   //tested
...
fn set_stop_all_status<S: Storage, A: Api, Q: Querier>(
    ...
    check_if_admin(&config, &env.message.sender)?;
    config.set_contract_status(ContractStatusLevel::StopAll);
...
fn set_stop_all_but_withdraw_status<S: Storage, A: Api, Q: Querier>(
    ...
    check_if_admin(&config, &env.message.sender)?;
    config.set_contract_status(ContractStatusLevel::StopAllButWithdraw);
...
fn try_deposit<S: Storage, A: Api, Q: Querier>(
    ...
    let _ = lottery(&mut deps.storage).save(&a_lottery);


    //Updating Supply Store
    let supply_store = TypedStoreMut::attach(&mut deps.storage);
...
fn claim_rewards<S: Storage, A: Api, Q: Querier>(
    ...
    let mut supply_pool: SupplyPool = TypedStoreMut::attach(&mut deps.storage).load(SUPPLY_POOL_KEY)?;
    ...
    //1 percent
    let triggerpercentage = constants.triggerer_share_percentage;
...
fn trigger_withdraw<S: Storage, A: Api, Q: Querier>(
    ...
    let mut supply_store = TypedStoreMut::attach(&mut deps.storage);
    let mut supply_pool: SupplyPool = supply_store.load(SUPPLY_POOL_KEY)?;
...
fn try_withdraw<S: Storage, A: Api, Q: Querier>(
    ...
    let mut results: Vec<(Uint128, u64)> = (user.requested_withdraw).into_iter().map(|(requested_withdraw_triggered, requested_time)|
        if requested_time + consts.unbonding_time <= env.block.time && withdraw_amount > available_for_withdraw {
...
fn try_redelegate<S: Storage, A: Api, Q: Querier>(
    ...
    let mut results: Vec<(Uint128, u64)> = (user.requested_withdraw).into_iter().map(|(requested_in_trigger, requested_time)|
        if requested_time + consts.unbonding_time <= env.block.time && amount_available_for_redelegation < redelegation_amount {
...
fn is_triggerer<S: Storage>(config: &Config<S>, account: &HumanAddr) -> StdResult<bool> {
    let consts = config.constants()?;
    if &consts.triggerer != account {
        return Ok(false);
    }
    Ok(true)
}

fn check_if_triggerer<S: Storage>(config: &Config<S>, account: &HumanAddr) -> StdResult<()> {
...
fn lottery_adjustment<S: Storage, A: Api, Q: Querier>(
    ...
    let mut a_lottery = lottery_read(&deps.storage).load().unwrap();
...
fn query_available_for_withdrawl<S: Storage, A: Api, Q: Querier>(
    ...
    let contract_balance = deps.querier.query_balance(constants.contract_address, "uscrt").unwrap().amount;
    ...
    for (requested_in_trigger, requested_time) in user.requested_withdraw {
        if (requested_time + constants.unbonding_time <= current_time) && amount_available_for_withdraw + requested_in_trigger <= contract_balance {
...
fn query_current_rewards<S: Storage, A: Api, Q: Querier>(deps: &Extern<S, A, Q>) -> StdResult<Binary> {
    ...
    let constants = ReadonlyConfig::from_storage(&deps.storage).constants()?;
    let pending_rewards = get_rewards(&deps.querier, &constants.contract_address).unwrap();
    let supply_pool = TypedStore::<SupplyPool, S>::attach(&deps.storage).load(SUPPLY_POOL_KEY)?;
...
fn change_validator<S: Storage, A: Api, Q: Querier>(
    ...
    let validator = consts.validator;
    consts.validator = address;
    config.set_constants(&consts)?;
    let supply: SupplyPool = TypedStore::attach(&deps.storage).load(SUPPLY_POOL_KEY)?;

    let mut messages: Vec<CosmosMsg> = vec![];
    messages.push(redelegate(&validator, &consts.validator,supply.total_tokens_staked));
...
fn change_unbonding_time<S: Storage, A: Api, Q: Querier>(
    ...
    let mut consts = config.constants()?;
    consts.unbonding_time =  unbonding_time;
    config.set_constants(&consts)?;
...
fn change_triggerer_share<S: Storage, A: Api, Q: Querier>(
    ...
    let mut consts = config.constants()?;
    consts.triggerer_share_percentage = percenatge;
    config.set_constants(&consts)?;
...
mod tests {
    ...
    fn testing_overall_deposit() {
        ...
        //checking lottery
        let a_lottery = lottery(&mut deps.storage).load().unwrap();
    ...
    fn testing_withdraw() {
        ...
        let env = mock_env("Batman", &[], 10, consts.unbonding_time);
    ...
    fn testing_redelegate() {
        ...
        let supply_store: SupplyPool = TypedStoreMut::attach(&mut deps.storage).load(SUPPLY_POOL_KEY).unwrap();
        assert_eq!(supply_store.total_tokens_staked.0, 1000000000);
    }
    ...
    fn testing_claim_rewards() {
        ...
        let config = Config::from_storage(&mut deps.storage);
        let consts = config.constants().unwrap();
        let _response = claim_rewards(&mut deps, mock_env("triggerer", &[], 10, consts.unbonding_time)).unwrap();
    ...
    fn test_change_admin_triggerer() {
        ...
        let mut config = Config::from_storage(&mut deps.storage).constants().unwrap();
    ...
    fn testing123() {
        ...
        let config = Config::from_storage(&mut deps.storage).constants().unwrap();
    ...
    fn testing_triggerer_rewards() {
        ...
        let mut config =  Config::from_storage(&mut deps.storage);
        let mut constants = config.constants().unwrap();
        constants.triggerer_share_percentage=1;
        config.set_constants(&constants);
    ...
    fn testing_triggerer_withdraw_rewards() {
        ...
        let config = Config::from_storage(&mut deps.storage);
        let consts = config.constants().unwrap();
        let _response = claim_rewards(&mut deps, mock_env("triggerer", &[], 10, consts.unbonding_time
        )).unwrap();
